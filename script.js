document.addEventListener('DOMContentLoaded', () => {
    ...
    const loginBtn = document.getElementById('login-btn'); // NUEVO: Obtener el botón de login
    ...
    // NUEVO: Event listener para el botón de login
    ...
    auth.onAuthStateChanged(user => {
        if (user) {
            ...
            appContainer.style.display = 'none';
            authContainer.style.display = 'none'; // Ocultar contenedor de autenticación inicialmente
            userProfileDropdown.style.display = 'block'; // Mostrar dropdown inicialmente
            userEmailDisplay.textContent = ''; // Limpiar email
            loginBtn.style.display = 'block'; // Mostrar botón de login
            logoutBtn.style.display = 'none'; // Ocultar botón de logout

            // NO llamar a ui.start() aquí. Se llamará cuando se haga clic en loginBtn.
        }
    });
    ...
    function initializeAppLogic(user) {
        ...
        const mainScript = document.getElementById('main-script'); // NUEVO: Obtener la etiqueta script
        ...
        versionRef.on('value', (snapshot) => {
            ...
            if (commitHash) {
                ...
                // --- NUEVO: Cache Busting para CSS y JS ---
                if (mainStylesheet) {
                    mainStylesheet.href = `style.css?v=${commitHash}`;
                }
                if (mainScript) { // NUEVO: Aplicar cache busting a script.js
                    mainScript.src = `script.js?v=${commitHash}`;
                }
            }
        });
